...
预测长度 \(H \in \{48, 96, 144, 192\}\)，回看窗口 \(T=2H\)。<span style="color:#c00;">**红色为最佳**</span>，<span style="color:#07c;">**蓝色为次优**</span>。

| 数据集 | H | FTS (MAE / MSE) | TS (MAE / MSE) | Koopa | PatchTST | TSMixer | DLinear | NSTformer | LightTS | Autoformer |
|---------|---|------------------|----------------|-------|----------|---------|---------|-----------|---------|------------|
...
基线随机种子为 2021*。**偏好率（Preference, %）** 同时衡量性能和稳定性，越高表示越偏好 FTS。

| 随机种子 | TS (MSE) | FTS (MSE) | 偏好率 (%) |
|------|----------|-----------|----------------|
| 2021* | 0.0929 | 0.0868 | — |
...
NP = 无扰动，NPO = NP + FTS 优化，IP = 输入扰动，IPO = IP + 优化，ILP = 中间层扰动，ILPO = ILP + 优化。**粗体**表示 FTS 改进后结果。

| 数据集 | H | NP | NPO | IP | IPO | ILP | ILPO |
|---------|---|-----|-----|-----|-----|-----|------|
...
## 热力图（来自论文）：预测长度 48、96、144、192

Exchange 数据集上的中间层张量（MSE）。Nper vs Oper 展示优化的有效性。
...
附录包含 **定理 1**（Sib 上界）与 **定理 2**（Sib 下界）的证明、随机种子稳定性表中使用的 **偏好率公式**，以及实验设置说明。选择输入扰动、层扰动和随机种子扰动，是因为它们与真实应用（如 Web 流量预测）直接相关：输入扰动反映活动和事件等外部因素；层扰动来自参数初始化与训练；随机种子扰动来自深度学习初始化。FTS 定义同时处理三者，以提升时间序列预测中的鲁棒性和稳定性。
